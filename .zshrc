...
_git-town()
{
    ...
    __git-town_debug "\n========= starting completion logic =========="
    __git-town_debug "CURRENT: ${CURRENT}, words[*]: ${words[*]}"
    ...
    # (We cannot use $CURSOR as its value does not work when a command is an alias.)
    words=("${=words[1,CURRENT]}")
    __git-town_debug "Truncated words[*]: ${words[*]},"

    lastParam=${words[-1]}
    lastChar=${lastParam[-1]}
    __git-town_debug "lastParam: ${lastParam}, lastChar: ${lastChar}"

    # For zsh, when completing a flag with an = (e.g., git-town -n=<TAB>)
    # completions must be prefixed with the flag
    setopt local_options BASH_REMATCH
    if [[ "${lastParam}" =~ '-.*=' ]]; then
        # We are dealing with a flag with an =
    ...
    if [ "${lastChar}" = "" ]; then
        # If the last parameter is complete (there is a space following it)
        # We add an extra empty parameter so we can indicate this to the go completion code.
        __git-town_debug "Adding extra empty parameter"
        requestComp="${requestComp} \"\""
    fi

    __git-town_debug "About to call: eval ${requestComp}"

    # Use eval to handle any environment variables and such
    out=$(eval ${requestComp} 2>/dev/null)
    __git-town_debug "completion output: ${out}"
    ...
    __git-town_debug "last line: ${lastLine}"
    ...
    if [ "${lastLine[1]}" = : ]; then
        ...
        # There is no directive specified.  Leave $out as is.
        __git-town_debug "No directive found.  Setting do default"
        directive=0
    fi

    __git-town_debug "directive: ${directive}"
    __git-town_debug "completions: ${out}"
    __git-town_debug "flagPrefix: ${flagPrefix}"

    if [ $((directive & shellCompDirectiveError)) -ne 0 ]; then
        __git-town_debug "Completion received error. Ignoring completions."
        return
    fi
    ...
    while IFS='\n' read -r comp; do
        # Check if this is an activeHelp statement (i.e., prefixed with $activeHelpMarker)
        if [ "${comp[1,$endIndex]}" = "$activeHelpMarker" ];then
            __git-town_debug "ActiveHelp found: $comp"
            comp="${comp[$startIndex,-1]}"
            if [ -n "$comp" ]; then
                compadd -x "${comp}"
                __git-town_debug "ActiveHelp will need delimiter"
                hasActiveHelp=1
            fi
        ...
        if [ -n "$comp" ]; then
            ...
            __git-town_debug "Adding completion: ${comp}"
            completions+=${comp}
            lastComp=$comp
        fi
    done < <(printf "%s\n" "${out[@]}")
    ...
    if [ $hasActiveHelp -eq 1 ]; then
        if [ ${#completions} -ne 0 ] || [ $((directive & shellCompDirectiveNoFileComp)) -eq 0 ]; then
            __git-town_debug "Adding activeHelp delimiter"
            compadd -x "--"
            hasActiveHelp=0
        fi
    fi
    ...
    if [ $((directive & shellCompDirectiveNoSpace)) -ne 0 ]; then
        __git-town_debug "Activating nospace."
        noSpace="-S ''"
    fi

    if [ $((directive & shellCompDirectiveKeepOrder)) -ne 0 ]; then
        __git-town_debug "Activating keep order."
        keepOrder="-V"
    fi
    ...
    if [ $((directive & shellCompDirectiveFilterFileExt)) -ne 0 ]; then
        ...
        __git-town_debug "File filtering command: $filteringCmd"
        _arguments '*:filename:'"$filteringCmd"
    elif [ $((directive & shellCompDirectiveFilterDirs)) -ne 0 ]; then
        # File completion for directories only
        local subdir
        subdir="${completions[1]}"
        if [ -n "$subdir" ]; then
            __git-town_debug "Listing directories in $subdir"
            pushd "${subdir}" >/dev/null 2>&1
        else
            __git-town_debug "Listing directories in ."
        fi
        ...
        __git-town_debug "Calling _describe"
        if eval _describe $keepOrder "completions" completions $flagPrefix $noSpace; then
            __git-town_debug "_describe found some completions"

            # Return the success of having called _describe
            return 0
        else
            __git-town_debug "_describe did not find completions."
            __git-town_debug "Checking if we should do file completion."
            if [ $((directive & shellCompDirectiveNoFileComp)) -ne 0 ]; then
                __git-town_debug "deactivating file completion"
                ...
                # Perform file completion
                __git-town_debug "Activating file completion"
...
alias jmn="cd ~/Development/jammin/"
